...
class MongoDFixture(interface.Fixture):
    ...
    def _do_teardown(self):
        if self.mongod is None:
            self.logger.warning("The mongod fixture was not set up.")
            return  # Still a success even if nothing is running.

        if not self.is_running():
            exit_code = self.mongod.poll()
            msg = ("mongod on port {:d} was expected to be running before teardown, but wasn't. "
        ...
        if exit_code == 0:
            self.logger.info("Successfully terminated the mongod on port {:d}".format(self.port))
        else:
            self.logger.warning("Terminated the mongod on port {:d}. "
                                "Process exited with code {:d}".format(self.port, exit_code))
